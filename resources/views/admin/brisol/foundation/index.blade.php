<x-app-layout>
    ...
    <x-slot name="header">
        <div x-data="{ open: false }" class="relative inline-block text-left font-poppins">
            ...
            <div x-show="open" @click.away="open = false" class="absolute left-0 w-56 mt-2 origin-top-right bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 z-50">
    ...
    <x-slot name="script">
        <script>
            var isAuthorized = @json(auth()->user()->hasAnyRole(['Super Admin', 'Admin Brisol']));
            // AJAX DataTable
            var datatable = $('#dataTable').DataTable({
                ...
                stateSave: false,
                ajax: {
                    url: '{{ route('admin.brisol.foundation.index') }}',
                    type: 'GET',
                },
                language: {
        ...
        <script>
            function downloadTemplate() {
                var templateUrl = '{{ asset('TemplateExcel/Template_Foundation_FAM_IEM.xlsx') }}';
                var link = document.createElement('a');
                link.href = templateUrl;
                link.download = 'Template_Foundation_FAM_IEM.xlsx';
                link.click();
            }
        </script>
                ...
                <div class="px-4 py-5 bg-white sm:p-6">
                    <div class="flex justify-between mb-10">
                        ...
                        <div class="button-container flex gap-4">
                            ...
                            @hasanyrole('Super Admin|Admin Brisol')
                                            ...
                                            <div class="sm:flex sm:items-start">
                                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                                    ...
                                                    <div class="mt-2">
                                                        ...
                                                        @endif
                                                        <form id="uploadForm" action="{{ route('admin.brisol.foundation.store') }}" method="POST" enctype="multipart/form-data">
                                                            @csrf
                                                            <div class="mb-3">
                        ...
                        <table id="dataTable" class="font-poppins font-medium text-sm rounded-table">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 15%;" class="px-6 py-3 bg-darker-blue text-left text-xs font-medium text-white uppercase tracking-wider">
                                        Operational Categorization Tier 1</th>
                                    <th style="width: 15%;" class="px-6 py-3 bg-darker-blue text-left text-xs font-medium text-white  uppercase tracking-wider">
                                        Operational Categorization Tier 2</th>
                                    <th style="width: 10%;" class="px-6 py-3 bg-darker-blue text-left text-xs font-medium text-white uppercase tracking-wider">
