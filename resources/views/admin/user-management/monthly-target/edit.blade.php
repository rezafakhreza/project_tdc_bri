  ...
  <div class="py-12 font-poppins">
    <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div class="p-6 bg-white rounded-xl">
            <h1 class="mb-10 text-2xl font-medium">Edit Monthly Target</h1>
            <form action="{{ route('admin.user-management.monthly-target.update', $monthlyTarget->id) }}" method="POST">
              @csrf
              @method('PUT')
                ...
                <div class="flex flex-wrap px-3 mt-4 mb-6 -mx-3">
                    <div class="w-full">
                        ...
                        <?php
                        $selectedMonth = $monthlyTarget->month;
                        ?>
                        <select name="month" id="month" class="w-full px-4 py-2 mb-4 bg-gray-200 border border-gray-200 rounded shadow-sm focus:outline-none focus:bg-white focus:border-gray-500">
                            @for ($i = 1; $i <= 12; $i++)
                                <option value="{{ $i }}" {{ $i == $selectedMonth ? 'selected' : '' }}>{{ strftime('%B', mktime(0, 0, 0, $i, 1)) }}</option>
                            @endfor
                        </select>
                    </div>
                    <div class="w-full">
                    ...
                    <div class="w-full">
                        ...
                        <input class="w-full px-4 py-2 mb-4 bg-gray-200 border border-gray-200 rounded shadow-sm focus:outline-none focus:bg-white focus:border-gray-500"
                               ...
                               step="0.01"
                               min="0" max="999.99"
                               value="{{ $monthlyTarget->monthly_target_value }}"
                               required>
                    </div>
                </div>
